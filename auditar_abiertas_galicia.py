...
# Editado 10/09/2026
#
# Audita las licitaciones de Galicia almacenadas en Supabase mediante
# scraping HTML, comprobando la fecha límite y el estado del procedimiento.
#
# Si una licitación ya no está abierta:
#    - Si Galicia es la única fuente -> elimina la licitación.
#    - Si tiene otras fuentes -> elimina únicamente "Galicia" de la columna fuente.
#
...
# ========================================================
# EXTRAER FECHA LÍMITE DEL HTML DE GALICIA
# ========================================================
...
def extraer_fecha_fin_html(soup):
    ...
    for dt in dts:
        ...
        if (
            "data e hora límite" in texto_dt
            or "data e hora limite" in texto_dt
            or "data límite" in texto_dt
            or "data limite" in texto_dt
            or "data de límite" in texto_dt
            or "data de limite" in texto_dt
        ):
    ...
    # ====================================================
    # 2. BUSCAR VARIANTES RELACIONADAS CON EL PLAZO
    # ====================================================
    ...
    for dt in dts:
        ...
        if (
            "data e hora" in texto_dt
            and (
                "límite" in texto_dt
                or "limite" in texto_dt
            )
        ):
    ...
    # ====================================================
    # 3. BÚSQUEDA DE SEGURIDAD EN TODO EL HTML
    # ====================================================
    ...
    patrones = [
        r"data\s+e\s+hora\s+l[ií]mite.{0,100}"
    ]
...
# ========================================================
# DETECTAR ESTADO DEL PROCEDIMIENTO DE GALICIA
# ========================================================

def detectar_estado_html(soup):

    """
    Extrae el estado del procedimiento de Galicia.
    ...
    # ====================================================
    # 1. BUSCAR "ESTADO DO PROCEDEMENTO"
    # ====================================================
    ...
    coincidencia = re.search(
        r"Estado\s+do\s+procedemento\s*:\s*([^|]+?)(?=\s{2,}|$)",
        texto_pagina,
        flags=re.IGNORECASE
    )
    ...
    if coincidencia:
        ...
        estado_extraido = coincidencia.group(
            1
        ).strip()
    ...
    for p in soup.find_all("p"):
        ...
        if (
            "estado do procedemento" in
            texto_p.lower()
        ):

            partes = re.split(
                r"estado\s+do\s+procedemento\s*:",
                texto_p,
                flags=re.IGNORECASE
            )
            ...
            if len(partes) > 1:
                ...
                if estado_extraido:
                    ...
                    # Si hay un <em>, normalmente contiene
    ...
    estados_mantener = [
        ...
        "aberto",
        "aberta",
        "aberto / prazo de presentación",
        "aberto / prazo de presentacion"
    ]
...
# ========================================================
# QUITAR GALICIA DE LA COLUMNA FUENTE
# ========================================================

def quitar_fuente_galicia(fuente_raw):
...
def auditar_licitaciones_html_galicia():
    ...
    print(
        ...
        "scraping HTML de Galicia...\n"
    )


    # ====================================================
    # 1. OBTENER LICITACIONES DE GALICIA
    # ====================================================
    ...
    # ====================================================
    # FILTRAR SOLO REGISTROS DONDE GALICIA
    ...
    for reg in registros:
        ...
        if any(
            ...
            registros_galicia.append(
                reg
            )

    registros = registros_galicia
    ...
    print(
        f"Se han encontrado "
        f"{total_registros} licitaciones de Galicia "
        f"para auditar."
    )
    ...
    if not registros:
        ...
        print(
            "\nNo hay licitaciones de Galicia "
            "para auditar."
        )
    ...
    for inicio in range(
        ...
        for reg in lote:
            ...
            try:
                ...
                # =================================================
                # 10. ESTADO QUE IMPLICA QUE GALICIA
                # YA NO ESTÁ ABIERTA
                # =================================================
    ...
    print(
        "AUDITORÍA DE GALICIA FINALIZADA"
    )
...
if __name__ == "__main__":
    ...
    auditar_licitaciones_html_galicia()
